// ==UserScript==
// @name        Canvas quiz all submissions information download
// @author      WenChen Hol
    ...
    try {
      ...
      $.getJSON(url, function (adata, status, jqXHR) {
        ...
        for (var id in tmpQuizSubmissions ) {
          ...
          //console.log( tmpItem.id, attemptAr, tmpItem.id in attemptAr );
          if ( !( tmpItem.id in attemptAr ) ){
            attemptAr[ tmpItem.id ] = [];
          }
          if( tmpItem["attempt"] >1 ) {
            //get first attempt time/score record
            var numAttempts = tmpItem["attempt"];
            if ( numAttempts > maxAtmps ){
                maxAtmps = numAttempts;
            }
            for ( var atmps=1;atmps<numAttempts; atmps++ ) {
                ...
                $.getJSON(tmpUrl, function (attemptData, status, jqXHR ) {
                  ...
                  //if ( ! (attemptData["quiz_submissions"][0].id in attemptAr) ) {
                   // attemptAr[ attemptData["quiz_submissions"][0].id ] = new Object();
                  //}
                  //Object.assign( attemptAr[ attemptData["quiz_submissions"][0].id ], {"time_spent_"+atmps:attemptData["quiz_submissions"][0].time_spent} );
                  //Object.assign( attemptAr[ attemptData["quiz_submissions"][0].id ], {"score_"+atmps:attemptData["quiz_submissions"][0].score} );
                  let newAtmps = tmpUrl.split( 'attempt=')[1];
...
function createQuizSubmissionCSV() {
    ...
    if ( maxAtmps >0 ) {
        for (var atmps=1;atmps<maxAtmps;atmps++) {
            fields.push( "time_spent_" + atmps );
        ...
        try {
            ...
            if ( maxAtmps >1 ) {
                ...
                for (let atmps=1;atmps<maxAtmps;atmps++) {
                   ...
                   if ( ( "time_spent_"+atmps ) in attemptAr[item.id] ){
                        divNum+=1;
                        timeSpent += attemptAr[item.id]["time_spent_"+atmps];
                        quizSubmissionReportAr[tmpId]["time_spent_"+atmps] = attemptAr[item.id]["time_spent_"+atmps];
                        quizSubmissionReportAr[tmpId]["score_"+atmps] = attemptAr[item.id]["score_"+atmps];
                        console.log( "time_spent_"+atmps + " in attemptAr:", item.id, (( "time_spent_"+atmps ) in attemptAr[item.id]) )
                   } else {
                       quizSubmissionReportAr[tmpId]["time_spent_"+atmps] = '';
                       quizSubmissionReportAr[tmpId]["score_"+atmps] = '';
                   }
                }
                if ( item['attempt']==1 || item['attempt']=="1" ){
                    quizSubmissionReportAr[tmpId][ "time_spent_1"] = item['time_spent'];
                    quizSubmissionReportAr[tmpId][ "score_1"] = item['score'];
                }
                try{
